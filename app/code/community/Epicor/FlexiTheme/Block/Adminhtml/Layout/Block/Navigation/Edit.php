...
class Epicor_FlexiTheme_Block_Adminhtml_Layout_Block_Navigation_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    ...
    public function getHeaderText()
    {
        if (Mage::registry('layout_block_data') && Mage::registry('layout_block_data')->getId()) {
            $title = Mage::registry('layout_block_data')->getBlockName();
             $header_text= Mage::helper('adminhtml')->__('Edit Navigation Block "%s"', $this->htmlEscape($title));
        } else {
            $header_text =  Mage::helper('adminhtml')->__('New Navigation Block');
        }
