...
Epicor_Locations.productLocations.prototype = {
    ...
    initialize: function(form, table, grid) {
        if ($(form)) {
            ...
            this.availableLocations = unserialize(this.form.select('input[name=available_locations]')[0].value);
    ...
    rowEdit: function(event, id) {
        if (typeof (id) == 'undefined') {
            id = event.element().up().up().select('input')[0].readAttribute('rel');
        }
        var rowData = null;
        if (id) {
...
document.observe('dom:loaded', function() { 
   $$("button.save").forEach(function(element){
        if (typeof (productLocations) != 'undefined' && productLocations.formWrapper != null) {
            element.writeAttribute("onclick", "if(productLocations.formWrapper.visible()){productLocations.rowUpdate();}" + element.readAttribute("onclick").replace('"', '\"'));
        }
    });
});
