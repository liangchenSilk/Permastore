		...
		miniColors: function(o, data) {
			...
			var create = function(input, o, data) {
				...
				var trigger = null;
                                ...
                                switch (color) {
                                    case 'transparent':
                                        trigger = jQuery('<a class="miniColors-trigger transparent" href="#"></a>');
                                        break;
                                    case '':
                                        trigger = jQuery('<a class="miniColors-trigger blank" href="#"></a>');
                                        break;
                                    default:
                                        // Determine initial color (defaults to white)
                                        trigger = jQuery('<a class="miniColors-trigger" style="background-color: #' + color + '" href="#"></a>');
                                        break;
                                }
			...
			var show = function(input) {
				...
				jQuery.each(swatches, function(key, swatchColor) {
				
					var swatchForeground = jQuery('<span></span>')
						.css('background-color', swatchColor);
					var swatch = jQuery('<div class="miniColors-colorSwatch"></div>')
						.data('swatch-color', swatchColor)
						.bind('click', function(e) {
							input.val(jQuery(this).data('swatch-color'));
							setColorFromInput(input);
						}).append(swatchForeground)
						;
					
					colorSwatches.append(swatch);
				});
				
				var swatch = jQuery('<div class="miniColors-colorSwatch blank"></div>')
					.data('swatch-color', '')
					.bind('click', function(e) {
						input.val(jQuery(this).data('swatch-color'));
						setColorFromInput(input);
				...
				jQuery(document).bind('mousedown.miniColors touchstart.miniColors', function(event) {
					...
					if( testSubject.hasClass('miniColors-selector') || ( hover_container != undefined && (hover_container.has(event.target).length !== 0 || hover_container.is(event.target))) ) {
						event.preventDefault();
						return;
					}
			...
			var setColor = function(input, hsb, updateInput) {
                                ...
                                input.siblings('.miniColors-trigger').removeClass('blank').removeClass('transparent');
			...
			var setColorFromInput = function(input) {
                                ...
                                input.siblings('.miniColors-trigger').removeClass('blank').removeClass('transparent').addClass(newClass);
