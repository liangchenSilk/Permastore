...
Epicor_Lists.listProduct.prototype = {
    ...
    pricing: function (row, event) {
        ...
        this.form_key = $('form_key').value;
        var data = {list: this.listId, product: productId, form_key: this.form_key};
        this.url = this.url + '?isAjax=true';
    ...
    togglePricingGrid: function (row, productId, pricing) {
        ...
        if (this.pricingIsEditable) {
            ...
            actionsDiv.insert('<span id="salesrep_rules_error_msg_' + productId + '" title="salesrep_rules_error_msg_' + productId + '" ></span>' ); 
        ...
        $('salesrep_rules_error_msg_' + productId).hide();         
    },
    addPricingRow: function (productId, data) {
        ...
        if (this.pricingIsEditable) {
            priceHtml = '<input id = "product_pricing_price_' + data.id + '"  name="product_pricing_price" value="' + data.price + '" class=" product_pricing_price required-entry validate-zero-or-greater input-text" type="text" onBlur="' + onBlur + '">';
        }
        ...
        if (this.pricingIsEditable) {
            var select = new Element('select', {id: 'product_pricing_currency_'+data.id ,name: 'product_pricing_currency', class: 'required-entry', onBlur: onBlur});
        ...
        $('pricing_row_' + data.id).observe("change", function(){  
            var price = this.down('.product_pricing_price').value;
            var currency = $('product_pricing_currency_' + data.id).value;
            listProduct.salesrepPricingRules(salesrepId, productId, price, currency, data.id);
        }) 
    },
    ...
    addBreak: function (type, dataId, productId, data) {
        ...
        if (this.pricingIsEditable) {
            if (type == 'price_breaks') {
                tr.insert('<td><input type="number" class="no-spinners" data-productId = "' + productId + '" name="break_qty" value="' + data.qty + '" onBlur="' + onBlur + '"/></td>');
            } else {
                tr.insert('<td><input type="number" step="any" class="no-spinners" name="break_value" value="' + data.value + '" onBlur="' + onBlur + '" /></td>');
            }
            tr.insert('<td><input type="number" step="any" class="no-spinners" name="break_price" value="' + data.price + '" onBlur="' + onBlur + '" /></td>');
            ...
            tr.insert('<td class="no-link"><span class="nobr">\n\
                    <button title="Delete" type="button" class="scalable delete icon-btn delete-product-option pricing-row-breaks-delete-button" onclick="return listProduct.removeBreak(this, \'' + productId + '\',\'' + dataId + '\');">\n\
                <span class="pricing-row-breaks-delete-button-span">' + this.translate('Delete') + '</span></button></span></td>');
        } else {
    ...
    salesrepPricingRules: function (salesrepId, productId, price, currency, listProductId) {    
        selected_erpaccount = '';
        if((typeof($('selected_erpaccount') != 'undefined') && $('selected_erpaccount') != null)) {
            selected_erpaccount = $('selected_erpaccount').value;
        }    
        var data = {salesrepId: salesrepId, product: productId, price: price, currency: currency, listId: this.listId, listProductId: listProductId, selectedErpaccount: selected_erpaccount ,form_key: this.form_key};
        var url = salesrepTestPricingRulesUrl + '?isAjax=true';
        new Ajax.Request(url, {
            ...
            onSuccess: function (request) {
                ...
                if(json.passed == false){
                    $("salesrep_rules_error_msg_" + productId).innerHTML = 'Price has Changed or Change is Outside Salesrep Limits';
                    $("product_pricing_price_" + listProductId).value = json.original_value;
                    $("salesrep_rules_error_msg_" + productId).show();
                }else{
                    $("salesrep_rules_error_msg_" + productId).innerHTML = '';
                    $("salesrep_rules_error_msg_" + productId).hide();
                }
            }.bind(this),
            onFailure: function (request) {
...
function listerpaccountsSelectAll()
{
    $$('#listerpaccountsGrid_table input[type=checkbox]').each(function (elem) {
...
function listerpaccountsUnselectAll()
{
    $$('#listerpaccountsGrid_table input[type=checkbox]').each(function (elem) {
        if (elem.checked) {
             elem.simulate('click');
...
function isANumberCheck(value, productId)
{
    if(isNaN(value)){
        $("salesrep_rules_error_msg_" + productId).innerHTML = 'Entered Field Must be a Number';
        $("salesrep_rules_error_msg_" + productId).show();
    }else{
        $("salesrep_rules_error_msg_" + productId).innerHTML = '';
        $("salesrep_rules_error_msg_" + productId).hide();
